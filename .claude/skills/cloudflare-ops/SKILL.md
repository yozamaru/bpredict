...
**静的生成は直近3シーズンまで。** Pages の1デプロイ20,000ファイル上限に対し、**1ルートあたり5ファイル**（`index.html` + RSC ペイロード4本。工程11a で実測）が生成される。3シーズンで約15,100（76%）、5シーズンでは約25,200（126%）で超過する。1ルートあたりのファイル数は Next.js のバージョンで変わるため、CI の件数検査を省略しない。
