...
**ボックススコアの `TeamID` が `club_source_ids` で解決できない試合を飛ばす**）。
...
#### ホスト名は1つにする（サイトと API を同一オリジンに置く）

| パス | 行き先 |
|---|---|
| `bpredict.naomaru.app/api/*` | **Workers**（`wrangler.toml` の `routes`） |
| それ以外（`/`、`/data/*`、静的アセット） | **Pages** |

**API を別サブドメインに置かない。** 同一オリジンにすることで次の3つが同時に片づく。

1. **CORS が不要になる。** 画面は `/api/v1/...` を相対パスで叩く。クロスオリジンなら
   プリフライトと `Access-Control-Allow-Origin` の設計が要り、しかも CORS は
   レート制限の代替にならない（下記）
2. **CSP が `default-src 'self'` のままでよい**（3.6）。別オリジンなら `connect-src` の
   追加が要る
3. **静的JSON（`/data/*`）と API のオリジンが揃う。** 画面は取得先のパスだけを
   替えればよく、URL の組み立てに環境ごとの分岐が出ない（3.7）

**`*.workers.dev` と `*.pages.dev` は残るが、正本ではない。** ゾーンを持たないため
WAF もレートリミットもキャッシュルールも効かない。公開する URL と、バッチが叩く
`API_BASE_URL` は独自ドメイン側に揃える。

**内部エンドポイントも同じホストに出る**（`/api/v1/internal/*`）。これは
`*.workers.dev` のときと変わらず、守りは Bearer と tipoff ガードである（3.4）。
**WAF のレートリミットは `/api/v1/` 全体に掛かるため、バッチもこの制限を受ける** —
1回の取り込みで内部APIを叩くのは試合ごとに2回であり、10秒あたり30の上限に
当たらない。
...
#### 飛ばした行は、理由ごとに別の counter で数える

**1つの counter に混ぜると、出力から原因を切り分けられない。** 2020-21 の取り込みで
`非リーグ戦=128` と出たが（他シーズンは1〜2件）、この counter は**2つの理由を
足していた**。

| 理由 | 何が起きたか | counter |
|---|---|---|
| その年度のクラブ一覧にない相手 | 選抜チーム・海外クラブ・下位リーグ | `非リーグ戦` |
| 見出しにも行にも日付がない | 日付を推測で埋めない（4.4 の日程ページの2つの形） | **`日付不明`** |
| 状態がサーバ側に書かれていない | 2018-19 CS の不要になった第3戦 | `状態不明` |

**同じ誤りを2度している。** v1.26 で `状態不明` を「非リーグ戦と混ぜない」と決めた
のに、既存の2つが混ざったまま残っていた。件数だけ出して理由を混ぜるのは、
**黙って取りこぼすのと調査可能性の点でほとんど同じである**（4.4 の冒頭と同じ理由）。

**要約行はこのジョブの結果を運用者が見る唯一の窓である。** 出力の形を
`test_summary_line_counts_each_skip_reason_separately` で固定する。

#### リーグ戦の絞り込みはクラブ名で行わない（v1.40）

**行のクラブ名は略称のことがある。** 2020-21 の実データでは、行が `千葉J` /
`横浜BC`、クラブ選択肢が `千葉ジェッツ` / `横浜ビー・コルセアーズ` で一致せず、
**実在する128試合を落としていた**（クラブ一覧の件数は20で正しかった）。要件 5.3 の
旧版が書いていた「改称は選択肢側も当該年度の名称になるため取りこぼさない」という
前提は、**改称ではなく略称によって破られた**。

| 判定に使うもの | 理由 |
|---|---|
| **ボックススコアの `TeamID`** | シーズン・改称・リーグ再編をまたいで不変（Phase 0）。略称にも表記ゆれにも影響されない |
| ~~日程ページのクラブ名~~ | 同じクラブが行と選択肢で別の表記を持つ |

**取り込みのクラブ解決は元から `TeamID` で行っていた**（`parse_boxscore(clubs=...)`）。
名前に依存していたのは絞り込みだけである。

**除外する試合のボックススコアを1回取得することになる。** シーズンあたり1〜2件で、
要件 5.2 の「取得は必要最小限」に対する増分は無視できる。解決できなかった試合は
**`不正` ではなく `非リーグ戦` に数える**（データの欠陥ではなく対象外であり、
連続失敗の打ち切り counter にも入れない）。

**日程側のクラブ名は連戦番号（`series_game_no`）にだけ使う。** ここは
「同じクラブが同じ表記で現れる」ことだけを要求し、正式名称である必要がない。
選択肢と一致すれば公式IDを、しなければ**行の名前をそのまま鍵にする**。
表記が季中で揺れると連戦番号が振り直るため、**行に現れた名前の種類数を出力に出す**
（20クラブのシーズンで21種類あれば、どれかが2通りで書かれている）。

**`非リーグ戦` は件数だけでは調査できない。** 選抜チームが混ざったのか、実在の
クラブを取りこぼしたのかが区別できない。したがって次の2つも出す。

| 出力 | 何が分かるか |
|---|---|
| `クラブ一覧=N` | その年度のクラブ選択肢の件数。**20クラブのはずが18なら、ここで分かる** |
| `クラブ一覧にない相手: <名前> / <名前>` | 選択肢と照合できなかった行の名前（重複を除き、出現順）。**v1.40 以降、これは飛ばす理由ではない** — 連戦番号の鍵が公式IDでない行を示す |

**クラブ名をログに出してよい。** `club_seasons.name` として DB に保存している事実で
あり、機密でも取得した本文でもない（絶対ルール4に触れない）。
...
**ただし `dry_run` のときは流さない。** この入力は「取得はするが D1 には書かない」で
あり、マスタ投入は D1 への書き込みである。1回あたり約200行を消費し、**枠は
00:00 UTC にしか戻らない**ため、診断のたびにその日に取り込める試合数が減る
（2026-09-26 に診断2回で400行＝3試合ぶんを無駄にした）。
`test_dry_run_writes_nothing_to_d1` が固定する。
...
| — | **現在地は `usePathname()` から導く。** 呼び出し側から渡すと渡し忘れが静かに通る — 実際に `layout.tsx` が引数なしで `<Header />` を描き、**全ページが「今日の予測」を現在地として出していた**（下線が動かず、`aria-current` も誤っていた）。`npm run test:nav` が生成物を読んで検査する |
...
# 10. Web デプロイ（**直接アップロード**。Git 連携は使わない）
cd web && npm run build
npx wrangler pages deploy web/out --project-name bpredict --branch main
```

**実装順序の都合で、API デプロイが backfill より先に来る。** 書き込み経路を `/internal/*` に一本化したため、API がないと backfill が実行できない。

#### Pages は直接アップロードにする（Git 連携を使わない）

**Cloudflare はプロジェクト作成後に方式を変更できない。** 2026-09-26 に
`wrangler pages project create` で作った時点で確定しており、運営者が直接
アップロードを選んだ。

| 観点 | 直接アップロード（採用） | Git 連携 |
|---|---|---|
| ビルド環境 | **GitHub Actions の1つだけ。** CI が通したものをそのまま配る | Cloudflare 側にもう1つ増える |
| Pages のビルド枠（月500回） | **消費しない** | 毎回消費する |
| 日次バッチとの噛み合い | 静的JSON を書き出す流れの延長で配れる（3.7） | コミットを検知して再ビルドが走る |
| デプロイの引き金 | `wrangler pages deploy` を明示的に叩く | push で自動 |

**自動化には Pages 編集スコープの API トークンが要る。** `CF_API_TOKEN` は
マイグレーション専用（D1 Edit）であり、これには使えない（基本設計 7.4）。
ワークフローから配るときは専用のトークンを GitHub Secrets に置く。**それまでは
手元から叩く。**

**`wrangler pages` を `web/` で実行しない。** Next.js を検出して
`opennextjs-cloudflare build` を走らせ、`.open-next/` を作る。この構成は
**採用していない**（ISR が R2/KV を要求し、絶対ルール5に触れる。要件 7章）。
残ると ESLint が生成物を lint して落ちるため `.gitignore` に入れてある。

#### アイコンはファビコンとホーム画面用に限る

**ヘッダーやページ本文にロゴを置かない。** 基本設計 6.2 は「ロゴ・選手写真を使わない
ため LCP 要素はテキストになる」ことを性能の前提にしている。ページ内に画像を置くと
この前提が崩れる。

| ファイル | 用途 | サイズ |
|---|---|---|
| `web/app/icon.png` | ファビコン（32×32） | 3.4KB |
| `web/app/apple-icon.png` | ホーム画面追加（180×180） | 46KB |

App Router の metadata 規約でファイルを置くだけで `<link rel="icon">` が出る。
**PWA（F-14）の 192 / 512 は manifest と一緒に工程15で入れる** — 使わない画像を
先に置かない。
