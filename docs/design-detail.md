...
1シーズンの所要は**実測で約35分**（日程29ページ ＋ 試合555件、3秒＋ジッタ）。**1日1シーズンに限る。** これは保守的な設定ではなく上限である — **1シーズンの D1 書き込みは実測 約65,800行**で、日次枠10万行の66%を使う（2シーズンで超過する。基本設計 7.5）。要件 5.2 の「初期の一括取得は複数日に分割して実行する」も同じ方向に効く。11シーズンで実質11日かかる。
