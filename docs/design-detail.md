...
class RateLimitedClient:
    MIN_INTERVAL   = 3.0        # 秒。短縮しない
    JITTER         = 1.0
    TIMEOUT        = 30
    MAX_RETRY      = 2
    MAX_BYTES      = 5 * 1024 * 1024
    DAILY_LIMIT    = 3000
    UA = "<name>/1.0 (+https://<domain>/about)"

    def get(self, url: str) -> str:
        self._check_daily_limit()
        self._wait()
        with httpx.stream("GET", url, headers={"User-Agent": self.UA},
                          timeout=self.TIMEOUT) as r:
            if r.status_code in (429, 503):
                raise AbortedByRemote(url, r.status_code)
            r.raise_for_status()
            buf = bytearray()
            ...
            return buf.decode(r.encoding or "utf-8")
```
...
`generateStaticParams` は**直近5シーズンの範囲のみ**を生成し、範囲外は `notFound()` を返す。Pages の1デプロイあたりファイル数上限は 20,000 で、1ルートにつき HTML と RSC ペイロードの2ファイルが出るため、全10シーズンでは 20,184 ファイルとなり超過する（直近5シーズンなら 10,184、51%）。
...
| 4b | 残りの `/internal/*`（`predictions` / `finalize` / `evaluate` / `summary` / `log` / `ratings` / `games` / `stats` / `entries` / `models` と GET 群） | tipoff 経過後に409、freeze が子 → 親の順で通る、`GET /internal/games/ingested`（工程6が使う）と `GET /internal/models/active`（工程9が使う）が応答する |
