...
行の器は `<a>` でなくてよい（中止・延期の行はリンクを持たない）。`<a>` のときだけ
`ScheduleKey` と行のIDの一致を検査し、リンクのない行は中止・延期に限って許す。
状態欄も得点欄も空の行は状態を推測せず飛ばし、`SchedulePage.unresolved` に数える。
**得点が入っている行は状態が空でも飛ばさない**（終了試合が静かに消えるため）。
`<script>` / `<style>` の中身は文字として読まない。詳細は詳細設計 4.4。
